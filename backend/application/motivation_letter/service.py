...
from domain.motivation_letter.schema import MotivationLetter, MotivationLetterRequest
...
class MotivationLetterService:
    def __init__(self, logger_name: str = "motivation_letter.service"):
        ...
        if data_dir_env:
            ...
            self.db_base_dir = Path(__file__).parent.parent.parent / "db"
    ...
    def _get_result_folder(self, company: str, job_title: str, tenant_id: str = "default-tenant") -> Path:
        timestamp = datetime.now().strftime("%Y-%m-%d_%H%M%S")
        safe_company = self._sanitize_filename(company)[:50]
        safe_job_title = self._sanitize_filename(job_title)[:50]
        safe_tenant = self._sanitize_filename(tenant_id)[:50]
        folder_name = f"{timestamp}_{safe_tenant}_{safe_company}_{safe_job_title}"
...
"""
    ...
    def persist(self, request: MotivationLetterRequest, letter: MotivationLetter, tenant_id: str = "default-tenant") -> MotivationLetter:
        self.logger.info(
    ...
    def finalize_temp_pdf(self, pdf_id: str, company: str, job_title: str, tenant_id: str = "default-tenant") -> tuple[Path, str]:
        ...
        result_folder = self._get_result_folder(company, job_title, tenant_id=tenant_id)
