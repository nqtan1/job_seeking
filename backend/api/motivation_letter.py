...
async def generate_motivation_letter(
    ...
    try:
        logger.info(
            "Received motivation letter request candidate=%s company=%s job_type=%s language=%s format=%s for tenant=%s",
        ...
        letter = service.persist(request, letter, tenant_id=tenant.tenant_id)
...
async def generate_temp_pdf_endpoint(
    request: MotivationLetterRequest,
    tenant: TenantContext = Depends(get_tenant_context)
) -> dict:
    ...
    from starlette.concurrency import run_in_threadpool
    try:
        logger.info("Generating temporary PDF motivation letter draft for tenant=%s", tenant.tenant_id)
...
async def get_temp_pdf_file(
    pdf_id: str,
    tenant: TenantContext = Depends(get_tenant_context)
):
    """Retrieve temporary compiled PDF file."""
    # Ensure client is fully authorized for this tenant context before serving file
    logger.info("Retrieving temporary compiled PDF file for tenant=%s", tenant.tenant_id)
...
async def compile_verbatim_endpoint(
    request: CompileVerbatimRequest,
    tenant: TenantContext = Depends(get_tenant_context)
) -> dict:
    """Compile provided cover letter text verbatim to a temporary typeset PDF inside /tmp."""
    import uuid
    try:
        logger.info("Compiling user's manual edited text verbatim to typeset PDF for tenant=%s", tenant.tenant_id)
        pdf_id = str(uuid.uuid4())
...
async def finalize_temp_pdf_endpoint(
    req: FinalizeRequest,
    tenant: TenantContext = Depends(get_tenant_context)
) -> dict:
    """Move temporary PDF files to permanent results folder."""
    try:
        logger.info("Finalizing temporary PDF draft for company: %s, tenant=%s", req.company, tenant.tenant_id)
        final_pdf_path, final_pdf_str = service.finalize_temp_pdf(req.pdf_id, req.company, req.job_title, tenant_id=tenant.tenant_id)
        return {
...
async def get_supported_formats(
    ...
    logger.info("Fetching supported formats for tenant=%s", tenant.tenant_id)
